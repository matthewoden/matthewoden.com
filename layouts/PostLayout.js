...
const editUrl = (fileName) => `${siteMetdata.siteRepo}/blob/master/data/notes/${fileName}`
const discussUrl = (slug) =>
  `https://mobile.twitter.com/search?q=${encodeURIComponent(
    `${siteMetdata.siteUrl}/notes/${slug}`
  )}`
...
export default function PostLayout({ children, frontMatter, next, prev }) {
  ...
  return (
    <SectionContainer>
      <BlogSeo url={`${siteMetdata.siteUrl}/notes/${frontMatter.slug}`} {...frontMatter} />
      <article>
          ...
          >
            ...
            <footer>
              <div className="text-sm font-medium leading-5 xl:divide-y divide-gray-700 xl:col-start-1 xl:row-start-2">
                {tags && (
                  <div className="py-4 xl:py-8">
                    <h2 className="text-xs tracking-wide uppercase text-gray-400">Tags</h2>
              ...
              <div className="pt-4 xl:pt-8">
                <Link href="/notes" className="text-blue-500 hover:text-blue-400">
                  &larr; Back to Notes
                </Link>
              </div>
            </footer>
